...
import { Play, Pause, Download, Loader2, X, GripVertical, Library, Mic, Plus, Sparkles, Shuffle, Send, Save, RotateCcw, Moon } from "lucide-react";
...
const ModularTrackBuilder = ({ refreshKey = 0 }: ModularTrackBuilderProps) => {
  ...
  const executeBuild = async (env: EnvironmentSettings) => {
    ...
    try {
      ...
      const finalBuffer = await audioEngine.mixWithBackgroundAndLoop(processed, bgBuffer, env.bgVolume, loopCount, freqBuffer, env.freqVolume);
