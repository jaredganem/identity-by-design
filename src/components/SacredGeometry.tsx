...
const SacredGeometry = () => (
  <div className="absolute inset-0 overflow-hidden pointer-events-none">
    {/* Outer ring */}
    <motion.div
      className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full border border-primary/15 animate-spin-slow"
    ...
    <motion.div
      ...
      style={{ background: "radial-gradient(circle, hsl(195 100% 29% / 0.1), transparent)" }}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    <div
      ...
      style={{ background: "radial-gradient(circle, hsl(197 90% 55% / 0.1), transparent)" }}
    />
    <div
      className="absolute bottom-[25%] right-[10%] w-48 h-48 rounded-full animate-pulse-slow"
      style={{ background: "radial-gradient(circle, hsl(195 100% 29% / 0.06), transparent)", animationDelay: "2s" }}
    />
  </div>
);
